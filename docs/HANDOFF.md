...
- Desktop grid rows are independent CSS Grid tracks: Column A row 3 (line chart) CANNOT influence row 5's height (Pendientes + table). Any future "make X stretch to fix alignment" idea is structurally dead without a per-column flex wrapper, which the doughnut's mobile-carousel DOM position forbids (ADR-0027/0028/0029 + 4.1 addendum). Don't re-litigate; it's measured and documented.
- Table markup is desktop/mobile shared since Iteration 4 (single 6-month table, `net_flow_series.slice(-6)`); the Stage-10-original `.table-desktop-split` (two 6-month halves) no longer exists.
